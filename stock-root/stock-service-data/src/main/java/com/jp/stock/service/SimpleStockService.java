...
public class SimpleStockService implements StockService {
	 ...
	 */
	@Autowired
	private SequenceGenerator generateUniqueSequence;
	...
	public BigDecimal getPERatio(String symbol, BigDecimal price) {
		...
		BigDecimal dividend = getDividendYield(symbol, price);
		 ...
		 Optional.ofNullable(dividend).filter(decimalValue-> BigDecimal.ZERO.compareTo(decimalValue) < 0).orElseThrow(()-> new StockMarketServiceException(
					BusinessErrorCode.JP003B.getId(),BusinessErrorCode.JP003B.getDescription(),new Throwable(StockServiceConstants.ILLEGAL_VALUE_OF_DIVIDEND + symbol)));
	...
	public void recordTrade(TradeBO tradeBO) {
    	...
    	Optional.ofNullable(tradeBO).filter(validTrade -> StringUtils.isNotBlank(validTrade.getStockSymbol())).orElseThrow(()-> new StockMarketServiceException(
					BusinessErrorCode.JP001B.getId(),BusinessErrorCode.JP001B.getDescription(),new Throwable(StockServiceConstants.ILLEGAL_TRADE_VALUE)));
	...
	public BigDecimal getVolumeWeightedStockPrice(String symbol) {
		...
		Optional.ofNullable(symbol).map(String::trim).filter(string -> !string.isEmpty()).orElseThrow(()-> new StockMarketServiceException(
				BusinessErrorCode.JP003B.getId(),BusinessErrorCode.JP003B.getDescription(),new Throwable(StockServiceConstants.STOCK_SYMBOL_NULL)));
		

		// Fetching all the trade with given Stock Symbol from trade region in
		// Gemfire
		List<Trade> tradeList = tradeDao
		...
		if (tradeList.isEmpty())
			throw new StockMarketServiceException(
					BusinessErrorCode.JP001B.getId(),BusinessErrorCode.JP001B.getDescription(),new Throwable(StockServiceConstants.TRADE_NOT_FOUND));
	...
	private Stock getValidStock(String stockSymbol) {
		...
		return Optional.ofNullable(stockDao.findOne(stockSymbol)).orElseThrow( () -> new StockMarketServiceException(
				BusinessErrorCode.JP001B.getId(),BusinessErrorCode.JP001B.getDescription(),new Throwable(StockServiceConstants.STOCK_NOT_FOUND + stockSymbol)));
	 ...
	 */
	private List<Trade> getTradeRecordsByTime(Collection<Trade> tradeCollection,
			int minutes) {
