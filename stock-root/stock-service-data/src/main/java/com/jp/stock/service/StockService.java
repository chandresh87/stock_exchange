...
/**
 * StockService interface exposes the method to operate on a given Stock It
...
public interface StockService {
	 ...
	 *            
	 */
	default BigDecimal calculateDividend(StockBO stock, BigDecimal price,DividendCalculator dividendCalculator)
	{
	...
	public static void checkPositive(BigDecimal value) {
	 ...
	 Optional.ofNullable(value).filter(decimalValue-> BigDecimal.ZERO.compareTo(decimalValue) < 0).orElseThrow(()-> new StockMarketServiceException(
				BusinessErrorCode.JP003B.getId(),BusinessErrorCode.JP003B.getDescription(),new Throwable(StockServiceConstants.FOUND_NON_POSITIVE_VALUE + value)));
